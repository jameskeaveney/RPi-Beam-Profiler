...
class MyCamera(picamera.PiCamera):
    ...
    def __init__(self, col="Red", speed=1, ExpMode="auto", roi=[0.0, 3.67, 0.0, 2.74]):
    ...
    def capture_image(self):
        ...
        self.roi_frac = [
            int(self.roi[0] / self.ccd_xsize),
            int(self.roi[1] / self.ccd_xsize),
            1 - int(self.roi[2] / self.ccd_ysize),
            1 - int(self.roi[3] / self.ccd_ysize),
        ]

        self.cropped_image = self.image[
            h * self.roi_frac[3] : h * self.roi_frac[2], w * self.roi_frac[0] : w * self.roi_frac[1]
        ]
        if self.background is not None:
            cropped_bg = self.background[
                h * self.roi_frac[3] : h * self.roi_frac[2], w * self.roi_frac[0] : w * self.roi_frac[1]
            ]
        ...
        self.imageX = self.cropped_image.sum(axis=0).astype(np.float64) / ch
        self.imageY = self.cropped_image.sum(axis=1).astype(np.float64) / cw
